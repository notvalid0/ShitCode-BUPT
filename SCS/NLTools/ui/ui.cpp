...
void TUI::genButtonEvent(bool buttonPushed) {
  if (!buttonPushed) {
    ...
    if (api_key.empty() || model_endpoint.empty() || model_name.empty()) {
      std::cerr << "Error: Config Not Set properly, plz fill all blanks in "
                   "config.json"
                << std::endl;
    }

    std::string prompt = LLMService::generatePrompt(input, mode, inputFile,
...
std::string getCmdFeedBack() {
  ...
  if (api_key.empty() || model_endpoint.empty() || model_name.empty()) {
    std::cerr
        << "Error: Config Not Set properly, plz fill all blanks in config.json"
        << std::endl;
  }

  std::string prompt =
      LLMService::generatePrompt(input, mode, inputFile, outputFile, outputDir);
  std::string request =
      LLMService::sendRequest(prompt, api_key, model_endpoint, model_name);
  std::string cmd = cmdExec::generateCmd(request);
...
std::string TUI::getUsrInput() {
  // 代码参考
  // https://arthursonzogni.github.io/FTXUI/examples_2component_2input_8cpp-example.html
  std::string input;
  ftxui::Component inputBlock =
      ftxui::Input(&input, "How you wanna do with this file?");
  return input;
}
...
void TUI::delCmd(std::string &cmd) { cmd = nullptr; }
