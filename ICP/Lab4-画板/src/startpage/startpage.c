...
void showStartPage(void) {
  ...
  // 时间控制
  float timer = 0.0f;
  float fadeInDuration = 2.0f;  // 2.0
  float showDuration = 1.0f;    // 1.0
  float fadeOutDuration = 1.0f; // 1.0
  ...
  while (!WindowShouldClose()) {
    ...
    if (timer < fadeInDuration) {
      ...
      imageAlpha =
          1.0f - (timer - fadeInDuration - showDuration) / fadeOutDuration;
    } else {
      break;
    }
    ...
    // 绘制缩放后的图片
    DrawTextureEx(startImage, (Vector2){(float)posX, (float)posY}, // 位置
                  0.0f,
                  scale, // 缩放
                  (Color){255, 255, 255, (unsigned char)(imageAlpha * 255)}
                  // AI提供
    );
