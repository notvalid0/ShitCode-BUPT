  ...
  while (!WindowShouldClose()) {
    ...
    if (mode == 0) {
      if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) ||
        ...
        drawCanvas(&mousecanvas, &previousMousePos, &isFirstDraw, mousePos,
                   brushSize, colors[colorUsed]);
      }
    ...
    if (!startDraw) {
      ...
      DrawTextureRec(mousecanvas.texture,
                     (Rectangle){0, 0, mousecanvas.texture.width,
                                 -mousecanvas.texture.height},
                     (Vector2){0, 0}, WHITE);
      ...
      // 绘制画笔光标
      if (mode == 0 || mode == 1)
        DrawCircle(GetMousePosition().x, GetMousePosition().y, brushSize,
                   colors[colorUsed]);
      else if (mode == 2)
        DrawCircle(GetMousePosition().x, GetMousePosition().y, 2.5f,
                   colors[colorUsed]);
