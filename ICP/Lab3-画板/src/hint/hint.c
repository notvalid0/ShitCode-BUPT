...
void drawPolyHint(bool *showPolyHint) {
    if (*showPolyHint) {
        DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), Fade(RAYWHITE, 0.8f));
        DrawRectangle(0, 185, GetScreenWidth(), 80, BLACK);
        DrawText("USE [ALT] + [5]-[9] to switch different sides of the polygon", (GetScreenWidth()-MeasureText("USE [ALT] + [5]-[9] to switch different sides of the polygon", 20))/2, 215, 20, RAYWHITE);
        *showPolyHint = timeCounter();
    }
}